...
import (
	...
	"github.com/minpeter/domain-expiry-lookup/lib"
)
...
func MainHandler(w http.ResponseWriter, r *http.Request) {
	...
	if authKey != "" {
		...
		if providedKey == "" {
			http.Error(w, "API key required. See: github.com/minpeter/domain-expiry-lookup", http.StatusForbidden)
			return
		}
		...
		if providedKey != authKey {
			http.Error(w, "Invalid API key. See: github.com/minpeter/domain-expiry-lookup", http.StatusForbidden)
			return
		}
	}
	...
	jsonWhois, err := jsoniter.Marshal(info)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
